...
{
    internal class CommonSqlTransactionProvider : ISqlTransactionProvider
    {
        ...
        public CommonSqlTransactionProvider(IDatabaseConnectionProvider prov, int defaultCommandTimeout, RetryPolicy retryPolicy)
        {
        ...
        public IDeferredSqlConnection GetDeferred() => new DeferredSqlConnection(_defaultCommandTimeout);
        ...
        public T ExecuteTxn<T>(Func<ISqlConnection, T> func, int? commandTimeout = null) => ExecuteRawTxn(func, commandTimeout);
        ...
        public T ExecuteRawTxn<T>(Func<IRawSqlConnection, T> func, int? commandTimeout = null)
        {
            return _retryPolicy.ExecuteAction(() =>
            {
                using (SqlConnection conn = _connProv.GetConnection())
                {
                    ...
                    using (var ldTxn = new CommonSqlTransaction(conn, commandTimeout ?? _defaultCommandTimeout))
                    {
                        try
                        {
        ...
        public async Task<T> ExecuteTxnAsync<T>(Func<ISqlConnection, Task<T>> func, int? commandTimeout = null) => await ExecuteRawTxnAsync(func, commandTimeout);
