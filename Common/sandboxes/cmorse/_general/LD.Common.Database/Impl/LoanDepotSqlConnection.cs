...
{
    internal class LoanDepotSqlConnection : ISqlConnection, IRawSqlConnection
    {
        private static readonly ILog log = LoanDepotLogManager.GetLogger();

        private readonly SqlConnection _conn;
        private readonly LoanDepotSqlTransaction _ldTxn;
        ...
        public LoanDepotSqlConnection(SqlConnection conn, LoanDepotSqlTransaction txn, int commandTimeout)
        {
            _conn = conn;
            _ldTxn = txn;
        ...
        private SqlCommand PrepareCmd(string commandText, CommandType type, SqlParameter[] parameters)
        {
            ...
            {
                log.Debug(cmd.CommandType == CommandType.StoredProcedure ?
                    StoredProcLogger.ExecToString(commandText, parameters) :
                    StoredProcLogger.RawExecToString(commandText, parameters)
                    );
            }
        ...
        public IList<T> ExecuteRawQueryList<T>(string sql, Func<IResult, T> func, params SqlParameter[] parameters)
        {
            return ExecuteRawQuery(sql, parameters).Select(func).ToList();
        }
