...
namespace LD.Common.WebApi.Client
{
    ...
    public class ApiClient
    {
        ...
        protected void Execute(IRestRequest request, params HttpStatusCode[] expectedStatusCodes)
        {
            logRequest(request);
        ...
        protected T Execute<T>(IRestRequest request, params HttpStatusCode[] expectedStatusCodes) where T : new()
        {
            ...
            return JsonUtil.DeserializeObject<T>(response.Content);
        }

        private void logRequest(IRestRequest request)
        {
            string parameters = string.Join(",", request.Parameters.Where(p => p.Type == ParameterType.UrlSegment || p.Type == ParameterType.QueryString)
                                                           .Select(p => string.Format("{0}={1}", p.Name, p.Value)));

            log.DebugFormat("Server Request: {0} {1}, {2}", request.Method, request.Resource, parameters);
        }
        ...
        private void logResponse(IRestResponse response)
        {
            ...
            log.DebugFormat("Server Response: {0}/{1} {2} {3}", (int)statusCode, statusCode, response.ContentType, response.Content);
        }
