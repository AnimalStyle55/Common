...
namespace Tests.LD.Common.Serialization
{
    ...
    public class XmlUtilUtilTest
    {
        ...
        {
            var b = XmlUtil.DeserializeFromFile<BasicObject>(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "data/BasicObject.xml"));
        ...
        {
            var b = XmlUtil.DeserializeFromStream<BasicObject>(new StreamReader(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "data/BasicObject.xml")).BaseStream);
        ...
        public void Test_Serialize()
        {
            ...
            Assert.AreEqual("<BasicObject><x>55</x><y>abc</y><z>1.25</z><v>Value2</v></BasicObject>", s);

            s = XmlUtil.Serialize(b, XmlUtil.SerializerSettings.OmitNamespaces | XmlUtil.SerializerSettings.NoIndent);

            Assert.AreEqual("<?xml version=\"1.0\" encoding=\"utf-8\"?><BasicObject><x>55</x><y>abc</y><z>1.25</z><v>Value2</v></BasicObject>", s);
        }
    }
}